...
def splitData(dataset, cv):
    ...
    with open(DIR + "/implicit/cv{0}/train.pkl".format(cv), 'wb') as fs:
        pickle.dump(train.tocsr(), fs)
    with open(DIR + "/implicit/cv{0}/train_time.pkl".format(cv), 'wb') as fs:
        pickle.dump(train_time.tocsr(), fs)

    with open(DIR + "/implicit/cv{0}/test.pkl".format(cv), 'wb') as fs:
        pickle.dump(test.tocsr(), fs)
    with open(DIR + "/implicit/cv{0}/valid.pkl".format(cv), 'wb') as fs:
        pickle.dump(valid.tocsr(), fs)

    with open(DIR + "/implicit/cv{0}/trust.pkl".format(cv), 'wb') as fs:
        pickle.dump(trust.tocsr(), fs)
    with open(DIR + "/implicit/cv{0}/category.pkl".format(cv), 'wb') as fs:
...
def filterData(dataset, cv):
    DIR = os.path.join(os.path.dirname(os.getcwd()), "dataset", dataset)
    #filter
    with open(DIR + "/implicit/cv{0}/train.pkl".format(cv), 'rb') as fs:
        train = pickle.load(fs)
    with open(DIR + "/implicit/cv{0}/test.pkl".format(cv), 'rb') as fs:
        test = pickle.load(fs)
    with open(DIR + "/implicit/cv{0}/valid.pkl".format(cv), 'rb') as fs:
        valid = pickle.load(fs)
    with open(DIR + "/implicit/cv{0}/category.pkl".format(cv), 'rb') as fs:
        category = pickle.load(fs)

    with open(DIR + "/implicit/cv{0}/train_time.pkl".format(cv), 'rb') as fs:
        train_time = pickle.load(fs)

    with open(DIR + "/implicit/cv{0}/trust.pkl".format(cv), 'rb') as fs:
        trust = pickle.load(fs)
...
def generateGraph2(dataset, cv):
    ...
    with open(DIR + "/implicit/cv{0}/categoryDict.pkl".format(cv), 'rb') as fs:
...
def createCategoryDict2(dataset, cv):
    ...
    with open(DIR + "/implicit/cv{0}/categoryDict.pkl".format(cv), 'wb') as fs:
...
def testNegSample(dataset, cv):
    ...
    with open(DIR + "/implicit/cv{0}/test_data.pkl".format(cv), 'wb') as fs:
        pickle.dump(test_data, fs)
    with open(DIR + "/implicit/cv{0}/valid_data.pkl".format(cv), 'wb') as fs:
